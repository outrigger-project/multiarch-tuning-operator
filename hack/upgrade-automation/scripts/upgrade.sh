...
step_update_go_mod() {
    ...
    # Extract BUILD_IMAGE from Makefile
    local build_image
    local makefile_path="Makefile"
    build_image=$(grep '^BUILD_IMAGE' "$makefile_path" | head -1 | awk -F'=' '{print $2}' | tr -d ' ?')
    echo "Detected BUILD_IMAGE: $build_image" >&2

    # Extract Go version from the container (keep full patch version)
    local go_version
    go_version=$(podman run --rm "$build_image" go version | awk '{print $3}' | sed 's/go//')
    echo "Container Go version: $go_version" >&2
    ...
    echo "Updating k8s.io dependencies to v${k8s_version}..." >&2
    local k8s_deps
    k8s_deps=$(grep -E '^\s+k8s\.io/' go.mod | grep -v '//' | awk '{print $1}' | sort -u)

    for dep in $k8s_deps; do
        echo "  Updating $dep..." >&2
        if go get "${dep}@v${k8s_version}" 2>&1; then
            echo "    ✅ Updated to v${k8s_version}" >&2
        else
            echo "    ⚠️  v${k8s_version} not available, using compatible version" >&2
        fi
    done
    ...
    for dep in $other_deps; do
        ...
        if go get -u "$dep" >/dev/null 2>&1; then
            ...
            # go get failed, clean up backups
...
step_update_vendor() {
    ...
    # Extract BUILD_IMAGE from Makefile to get expected Go version
    local build_image
    local makefile_path="Makefile"
    build_image=$(grep '^BUILD_IMAGE' "$makefile_path" | head -1 | awk -F'=' '{print $2}' | tr -d ' ?')
    ...
    # Set GOTOOLCHAIN to use local version
    export GOTOOLCHAIN="local"
    echo "Setting GOTOOLCHAIN=local to preserve Go version $go_version" >&2
    echo "" >&2
...
main() {
    ...
    kustomize_version=$(discover_kustomize_version "$go_version")
    controller_tools_version=$(discover_controller_tools_version "$k8s_version" "$go_version")
    setup_envtest_version=$(derive_setup_envtest_version "$controller_runtime_version")
    envtest_k8s_version=$(extract_envtest_k8s_version "$controller_runtime_version")
    golint_version=$(discover_golangci_lint_version "$go_version")
    ...
    echo ""
    step_update_go_mod "$k8s_version" "$ocp_version" "$controller_runtime_version"
    echo ""
    step_update_vendor
    echo ""
    step_run_code_generation

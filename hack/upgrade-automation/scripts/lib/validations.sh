...
# Validate getCorrectHostmountAnyUIDSCC function
validate_hostmount_scc_function() {
    local k8s_version="$1"
    local ocp_version="$2"

    echo "Validating getCorrectHostmountAnyUIDSCC function..." >&2

    local file="controllers/podplacement/podplacement_controller.go"

    if ! [[ -f "$file" ]]; then
        echo "ℹ️  $file not found, skipping validation" >&2
        return 0
    fi

    if ! grep -q "getCorrectHostmountAnyUIDSCC" "$file"; then
        echo "ℹ️  getCorrectHostmountAnyUIDSCC function not found, skipping validation" >&2
        return 0
    fi

    echo "⚠️  MANUAL REVIEW REQUIRED:" >&2
    echo "   File: $file" >&2
    echo "   Function: getCorrectHostmountAnyUIDSCC" >&2
    echo "   " >&2
    echo "   This function maps K8s versions to OCP versions for SCC selection." >&2
    echo "   Please verify the version logic is correct for:" >&2
    echo "     - K8s $k8s_version" >&2
    echo "     - OCP $ocp_version" >&2
    echo "" >&2

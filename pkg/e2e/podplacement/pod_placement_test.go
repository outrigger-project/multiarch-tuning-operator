	...
	Context("the Preferred Node Affinity is correctly set with PodPlacementConfigs", func() {
		...
		It("Should respect priority when multiple PPCs configure the same architecture", func() {
			...
			d := NewDeployment().
				...
				Build()
			Expect(client.Create(ctx, d)).To(Succeed())
			By("Verifying the preferred affinity uses higher-priority PPC's weight and has-duplicates label is set")
			Eventually(framework.VerifyPodLabelsAreSet(ctx, client, ns, "app", "test",
				utils.SchedulingGateLabel, utils.SchedulingGateLabelValueRemoved,
				utils.PreferredNodeAffinityLabel, utils.NodeAffinityLabelValueSet,
				utils.PreferredNodeAffinitySourceLabel, utils.LabelValueSetWithDuplicates,
		...
		It("Should handle user-defined preferred affinity conflicting with PPC", func() {
			var err error
			By("Create an ephemeral namespace")
			ns := framework.NewEphemeralNamespace()
			err = client.Create(ctx, ns)
			...
			//nolint:errcheck
			defer client.Delete(ctx, ns)
			By("Creating a PodPlacementConfig that sets ppc64le")
			ppc := NewPodPlacementConfig().
				...
				Build()
			Expect(client.Create(ctx, ppc)).To(Succeed())
			By("Creating a deployment with user-defined ppc64le preference")
			archLabelPST := NewPreferredSchedulingTerm().WithArchitecture(utils.ArchitecturePpc64le).WithWeight(10).Build()
			ps := NewPodSpec().
				WithContainersImages(helloOpenshiftPublicMultiarchImage).
			...
			By("The pod should have been set node affinity of arch info.")
			Eventually(framework.VerifyPodNodeAffinity(ctx, client, ns, "app", "test", *expectedNSTs), e2e.WaitShort).Should(Succeed())
			By("The pod should keep user's ppc64le preference and add CPPC's arm64")
			amd64Term := NewPreferredSchedulingTerm().
				WithArchitecture(utils.ArchitectureAmd64).WithWeight(50).Build()
			ppc64leTerm := NewPreferredSchedulingTerm().
				WithArchitecture(utils.ArchitecturePpc64le).WithWeight(10).Build()
			Eventually(framework.VerifyPodPreferredNodeAffinity(ctx, client, ns, "app", "test",
				NewPreferredSchedulingTerms().
					WithPreferredSchedulingTerm(amd64Term).
					WithPreferredSchedulingTerm(ppc64leTerm).
		...
		It("Should set has-duplicates label when PPC partially overlaps with other PPCs", func() {
			By("Create an ephemeral namespace")
			ns := framework.NewEphemeralNamespace()
			err := client.Create(ctx, ns)
			Expect(err).NotTo(HaveOccurred())
			...
			d := NewDeployment().
				...
				Build()
			Expect(client.Create(ctx, d)).To(Succeed())
			By("Verifying has-duplicates label is set and labels are correct")
			Eventually(framework.VerifyPodLabelsAreSet(ctx, client, ns, "app", "test",
				utils.SchedulingGateLabel, utils.SchedulingGateLabelValueRemoved,
				utils.PreferredNodeAffinityLabel, utils.NodeAffinityLabelValueSet,
				utils.PreferredNodeAffinitySourceLabel, utils.LabelValueSetWithDuplicates,
		...
		It("Should set has-duplicates label when user preference conflicts with PPC and CPPC", func() {
			var err error
			By("Create an ephemeral namespace")
			ns := framework.NewEphemeralNamespace()
			err = client.Create(ctx, ns)
			...
			//nolint:errcheck
			defer client.Delete(ctx, ns)
			By("Creating a PodPlacementConfig that sets conflicts with user defined preferred node affinity")
			ppc := NewPodPlacementConfig().
				WithName("test-ppc-conflict").
				WithNamespace(ns.Name).
				WithPriority(100).
				...
				Build()
			Expect(client.Create(ctx, ppc)).To(Succeed())
			By("Creating a deployment with user-defined preference that conflicts with CPPC")
			...
			By("The pod should have been set node affinity of arch info.")
			Eventually(framework.VerifyPodNodeAffinity(ctx, client, ns, "app", "test", *expectedNSTs), e2e.WaitShort).Should(Succeed())
			By("The pod should keep only user's arm64 preference")
			archLabelPST = NewPreferredSchedulingTerm().WithArchitecture(utils.ArchitectureAmd64).WithWeight(5).Build()
			Eventually(framework.VerifyPodPreferredNodeAffinity(ctx, client, ns, "app", "test",
				NewPreferredSchedulingTerms().WithPreferredSchedulingTerm(archLabelPST).Build()), e2e.WaitShort).Should(Succeed())
		})

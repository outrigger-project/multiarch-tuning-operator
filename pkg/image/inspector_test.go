...
func Test_shouldSkipManifest(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Check for attestation manifest or unknown architecture
			shouldSkip := (tt.annotations != nil && tt.annotations["vnd.docker.reference.type"] == "attestation-manifest") ||
				(tt.platform != nil && tt.platform.Architecture == "unknown")
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, m := range tt.manifests {
				// Skip attestation manifests - they are not runnable images

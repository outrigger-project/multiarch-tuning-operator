...
func Test_shouldSkipManifest(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Check for attestation manifest
			if tt.annotations != nil && tt.annotations["vnd.docker.reference.type"] == "attestation-manifest" {
				shouldSkip = true
			}

			// Check for unknown architecture
			if !shouldSkip && tt.platform != nil && tt.platform.Architecture == "unknown" {
				shouldSkip = true
			}

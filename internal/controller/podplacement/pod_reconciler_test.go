		...
		Context("the Preferred Node Affinity is correctly set with PodPlacementConfigs", func() {
			...
			It("should merge multiple PPCs with priority-based deduplication when architectures overlap", func() {
				...
				Eventually(func(g Gomega) {
					...
					g.Expect(pod.Labels).To(HaveKeyWithValue(utils.NodeAffinityLabel, utils.NodeAffinityLabelValueSet),
						"node affinity label not found")
					annotation := pod.Annotations[utils.PreferredNodeAffinitySourcesAnnotation]
					g.Expect(annotation).To(ContainSubstring("amd64:50 from PodPlacementConfig-test-ppc-high (applied)"),
						"should show high-priority PPC applied amd64")
					g.Expect(annotation).To(ContainSubstring("arm64:60 from PodPlacementConfig-test-ppc-high (applied)"),
						"should show high-priority PPC applied arm64")
					g.Expect(annotation).To(ContainSubstring("arm64:30 from PodPlacementConfig-test-ppc-low (skipped)"),
						"should show low-priority PPC skipped arm64 due to higher priority")
					g.Expect(annotation).To(ContainSubstring("ppc64le:40 from PodPlacementConfig-test-ppc-low (applied)"),
			...
			It("should handle three PPCs with cascading priorities and overlapping architectures", func() {
				...
				Eventually(func(g Gomega) {
					...
					g.Expect(annotation).To(ContainSubstring("amd64:50 from PodPlacementConfig-test-ppc2 (skipped)"),
						"should show PPC2 skipped amd64:50 due to higher priority")
					g.Expect(annotation).To(ContainSubstring("arm64:60 from PodPlacementConfig-test-ppc2 (applied)"),
						"should show PPC2 applied arm64:60")
					g.Expect(annotation).To(ContainSubstring("arm64:25 from PodPlacementConfig-test-ppc3 (skipped)"),
						"should show PPC3 skipped arm64:25 due to higher priority")
					g.Expect(annotation).To(ContainSubstring("ppc64le:30 from PodPlacementConfig-test-ppc3 (applied)"),

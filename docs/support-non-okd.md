...
## Add Pull Secret
The operator is currently hardcoded to watch the `pull-secret` Secret in the `openshift-config` namespace. As a result, if you're running on a non-OpenShift cluster or the namespace does not exist, you need to manually create the `openshift-config` namespace and add a `pull-secret` Secret to it.
